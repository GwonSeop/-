...
import pandas as pd
data1 = pd.read_csv('C:/Users/ADMIN/Desktop/proDS/Dataset/Dataset_01.csv')
...
data1.isna().sum().sum()  ## 셀의 갯수
(data1.isna().sum(axis=1) >=1).sum() ## 행의 갯수 
##  26
...
# =============================================================================
data1.columns
val_list = data1.columns.drop('Influencer')
q2 = data1.corr().abs().drop('Sales')

q2.max()
q2.argmax() # 인덱스 번호
q2.idmax() # 인덱스 명
q2.nlargest(1) # 최대값 + 인덱스 명
##0.9995
...
#ols2 = ols1.fit()      #변수명이 달라야한다
ols1 = ols(form2,data=q3).fit()
dir(ols1)
ols1.summary()
(ols1.outlier_test()['unadj_p'] < 0.05).sum() ##q밸류값 이상치 갯수 확인
q3[(ols1.outlier_test()['unadj_p'] < 0.05)] ## 확인
dir(ols1)
ols1.params
#Intercept      -0.133963
#TV              3.562570
...
data2=pd.read_csv('C:/Users/ADMIN/Desktop/proDS/Dataset/Dataset_02.csv')
data2.isna().sum()
data2.info()
data2.columns
#%%
...
q2['Age_gr']= np.where(q2['Age']<20,'10',np.where(q2['Age']<30,'20',np.where(q2['Age']<40,'30',np.where(q2['Age']<50,'40',np.where(q2['Age']<60,'50','60')))))
...
(2.119248418109203,    # chi2 통계량
 0.7138369773987128,   # p-value  0.05보다 큼으로 독립이다
 4, # 자유도, (r-1)(c-1)
 array([[43.68, 11.04,  7.68,  7.68, 25.92],
        [47.32, 11.96,  8.32,  8.32, 28.08]])) # 독립인경우 기대 빈도
p_value = chi2[1]
...
# 5. Drug과 연관성이 있는 변수 추출 후 그 중 가장 높은 p-value
...
plot_tree(dt, max_depth=2, feature_names=x_list,
          class_names=y_label,
          precision=3,
          fontsize=5)
...
# =============================================================================
data3=pd.read_csv('C:/Users/ADMIN/Desktop/proDS/Dataset/Dataset_03.csv')
...
from scipy.stats import ttest_1samp, ttest_ind, ttest_rel, bartlett # ttest_1samp 일표본, ttest_ind 독립인 이표본, ttest_rel 대응인 이표본
...
data4 = pd.read_csv('C:/Users/ADMIN/Desktop/proDS/Dataset/Dataset_04.csv')
#(1) 한국 데이터만 필터 
df = data4[data4['LOCATION']=='KOR']


# (2) 연도별 육류 소비량 집계
q1_tab=pd.pivot_table(df, index='TIME',values='Value',aggfunc='sum').reset_index()
q1_tab.corr()
# 0.96
...
df = data4[(data4['LOCATION']=='KOR')|(data4['LOCATION']=='JPN')]
q1_tab=pd.pivot_table(df, index=['TIME','SUBJECT','LOCATION'],values='Value',aggfunc='sum').reset_index()

# 1 한국과 일본 데이터 필터
df = data4[data4.LOCATION.isin(['KOR','JPN'])]
# 2 육류 종류 연도별로 대응
sub_list=df.SUBJECT.unique()
# 3 대응 t - test (H0: 평균차이가 없다. H1: 있다)
from scipy.stats import ttest_rel
l_=[]
for i in sub_list:
    temp = df[df.SUBJECT == i]
    q2_tab=pd.pivot_table(temp, index='TIME',columns='LOCATION',
                   values='Value').dropna()
...
for i in sub_list:
    ...
    q_.append([i,r2_score,mape])
q_
#5.78

#temp['TIME'].values.reshape(-1,1)     (n,1) 쉐잎으로
#temp[['TIME']]
